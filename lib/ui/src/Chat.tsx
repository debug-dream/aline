...
import { isDefined, pluralize } from '@sourcegraph/cody-shared/src/common'
...
import { SystemPromptPicker } from './chat/SystemPromptPicker'
...
export const Chat: React.FunctionComponent<ChatProps> = ({
    ...
    const onChatSubmit = useCallback((text: string): void => {
        // Submit chat only when input is not empty and not in progress
        if (!messageInProgress) {
            setInputRows(1)
            submitInput(text, 'user')
            setFormInput('')
        }
    ...
    function setSystemPrompt(value: SystemPrompt): void {
    }
