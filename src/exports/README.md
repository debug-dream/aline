...
3. Get access to the API with the following code:
    ...
    ```ts
    const clineExtension = vscode.extensions.getExtension<ClineAPI>("saoudrizwan.aline-dev")
    ...
    **Note:** To ensure that the `saoudrizwan.aline-dev` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

    ```json
    "extensionDependencies": [
        "saoudrizwan.aline-dev"
    ]
    ```
